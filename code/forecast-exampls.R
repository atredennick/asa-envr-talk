...
outcasts_combined <- as.data.frame(outcasts_all) %>%
  mutate(year = 0:n_times) %>%
  gather(key,value,-year) %>%
  mutate(simulation = "A) All sources of error (F)") %>%
  rbind(as.data.frame(outcasts_noinit) %>%
          mutate(year = 0:n_times) %>%
          gather(key,value,-year) %>%
          mutate(simulation = "B) Parameter error only (PA)")) %>%
  rbind(as.data.frame(outcasts_noparam) %>%
          mutate(year = 0:n_times) %>%
          gather(key,value,-year) %>%
          mutate(simulation = "C) Initial conditions error only (I)"))
...
ggplot(outcasts_combined, aes(x = year, y = value, group = key))+
  ...
  theme_few()+
  theme(axis.text.y = element_blank())
ggsave(filename = "../figures/forecast_uncertainty_example2.pdf", width = 9, height = 3, units = "in")
